                ...
                <div key={exercise.id}>
                  ...
                  <div className="flex flex-col gap-1">
                    {Array.from(
                      ...
                      const setRows = exercise.completedSets.filter(
                        (cs) => cs.setNumber === setNumber,
                      );
                      const rpeRow = setRows.find(
                        (cs) => cs.dimension === 'rpe',
                      );
                      ...
                      return (
                        <div
                          key={setNumber}
                          className="flex gap-3 items-center flex-wrap text-xs"
                        >
                          <span className="text-gray-500 w-12">
                            Set {setNumber}
                          </span>
                          {exercise.targets.map((target) => {
                            const actual = setRows.find(
                              (cs) => cs.dimension === target.dimension,
                            );
                            return (
                              <span key={target.dimension}>
                                {labelFor(target.dimension)}:{' '}
                                {target.targetValue}
                                {unitFor(target.dimension)} planned
                                {actual
                                  ? ` → ${actual.value}${unitFor(target.dimension)} actual`
                                  : ' (not logged)'}
                              </span>
                            );
                          })}
