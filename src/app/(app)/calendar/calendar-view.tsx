    ...
    <div className="flex gap-6">
      <div className="flex-1">
        <div className="flex justify-between items-center mb-4">
          <button onClick={() => setMonthDate((d) => new Date(d.getFullYear(), d.getMonth() - 1, 1))} className="text-sm underline">
            ← Previous
          </button>
          <h2 className="font-medium">{monthDate.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</h2>
          <button onClick={() => setMonthDate((d) => new Date(d.getFullYear(), d.getMonth() + 1, 1))} className="text-sm underline">
            Next →
          </button>
        </div>
      ...
      <div className="w-64 shrink-0 border-l pl-4">
        {!selectedDayKey && <p className="text-gray-500 text-sm">Select a day to see its schedule.</p>}
                  ...
                  <div key={hour} className="flex items-center gap-2 text-sm">
                    ...
                    {workout ? (
                      ...
                      >
                        ...
                        <button onClick={() => handleUnschedule(workout.id)} className="text-red-600 ml-1">
                          ×
                        </button>
                      </div>
                    ) : (
      ...
      {selectedHour !== null && selectedDayKey && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4" onClick={() => setSelectedHour(null)}>
          <div className="bg-white rounded p-4 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-medium mb-3">
              Schedule a workout — {selectedDayKey} {String(selectedHour).padStart(2, '0')}:00
            </h3>
            <form onSubmit={handleSchedule} className="flex flex-col gap-3">
              <select value={templateId} onChange={(e) => setTemplateId(e.target.value)} required className="border rounded p-2">
