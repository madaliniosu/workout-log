...
type ScheduledExercise = { id: string; exerciseName: string; setCount: number; targets: Target[] };
type ScheduledWorkout = { id: string; name: string; scheduledAt: string; exercises: ScheduledExercise[] };
...
export function WorkoutCard({ workout, onLogged }: { workout: ScheduledWorkout; onLogged: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border rounded p-4 flex flex-col gap-4">
      ...
      <div className="flex gap-2">
        ...
        <button type="submit" disabled={!started || pending} className="border rounded p-2 disabled:opacity-50 px-4">
            ...
            {Array.from({ length: exercise.setCount }, (_, i) => i + 1).map((setNumber) => (
              <div key={setNumber} className="flex gap-3 items-center flex-wrap text-xs border-b pb-2">
                ...
                <label className="flex items-center gap-1">
                  ...
                  <select
                    value={values[exercise.id]?.[setNumber]?.rpe ?? ''}
                    onChange={(e) => updateValue(exercise.id, setNumber, 'rpe', e.target.value)}
                    required
