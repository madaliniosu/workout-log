...
export async function updateWorkoutTemplate(
  ...
  const exerciseRows = await db
    .select({ id: exercises.id, name: exercises.name })
    .from(exercises)
    .where(inArray(exercises.id, data.exercises.map((e) => e.exerciseId)));
  const nameById = new Map(exerciseRows.map((e) => [e.id, e.name]));
  ...
  await db.batch([
    db.update(workoutTemplates).set({ name: data.name, notes: data.notes }).where(eq(workoutTemplates.id, templateId)),
    db.delete(workoutTemplateExercises).where(eq(workoutTemplateExercises.templateId, templateId)),
    db.insert(workoutTemplateExercises).values(
      templateExercises.map(({ id, exerciseId, exerciseName, setCount, exerciseOrder }) => ({
        id,
        templateId,
        exerciseId,
        exerciseName,
